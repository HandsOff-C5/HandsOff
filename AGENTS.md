...
corepack pnpm build        # tsc build across packages
```
...
`format` · `lint` · `typecheck` · `test` · `build` · `secret-scan`. All must pass before review. The macOS desktop artifact builds on merge to `main` (and on demand), not as a per-PR blocker yet.
