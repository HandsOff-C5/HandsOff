...
extern "C" {
    ...
    fn handsoff_request_camera_authorization() -> i32;
}
...
fn request_app_media_permissions() -> Value {
    ...
    // Camera is requested from the app process so the prompt fires and the grant
    // registers under com.handsoff.desktop; the head-track sidecar inherits it.
    let speech = unsafe { handsoff_request_speech_authorization() };
    let microphone = unsafe { handsoff_request_microphone_authorization() };
    let camera = unsafe { handsoff_request_camera_authorization() };
    permissions_value(
        speech_authorization_state(speech),
        microphone_authorization_state(microphone),
        camera_authorization_state(camera),
    )
}

#[cfg(not(target_os = "macos"))]
fn request_app_media_permissions() -> Value {
    permissions_value("unknown", "unknown", "unknown")
}
...
fn camera_authorization_state(status: i32) -> &'static str {
    match status {
        0 => "not-determined",
        1 => "restricted",
        2 => "denied",
