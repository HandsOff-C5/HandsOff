...
describe("EDUCATED_PERMISSION_IDS", () => {
  it("covers the macOS TCC grants that gate head pointing and computer-use actions", () => {
    expect([...EDUCATED_PERMISSION_IDS]).toEqual([
      "camera",
      "accessibility",
      "screen-recording",
      "input-monitoring",
...
describe("permissionEducation — Input Monitoring", () => {
  it.each(NOT_READY)("returns targeted guidance when %s", (level) => {
    const guidance = permissionEducation(readiness("input-monitoring", level));
    expect(guidance).toBeDefined();
    expect(guidance?.settingsPath).toContain("Input Monitoring");
    expect(guidance?.reason).toContain("Right Option");
    expect(guidance?.steps.length).toBeGreaterThan(0);
  });

  it("returns no guidance once granted", () => {
    expect(permissionEducation(readiness("input-monitoring", "ready"))).toBeUndefined();
  });
});
...
describe("permissionSetupState", () => {
  ...
  it("pairs each not-ready grant with its guidance, in display order", () => {
    const state = permissionSetupState(report("blocked", "blocked", "attention", "attention"));
    ...
    expect(state.toGrant[3]?.guidance.settingsPath).toContain("Input Monitoring");
  });

  it("surfaces only the blocked grant when the other is ready", () => {
    const state = permissionSetupState(report("ready", "ready", "blocked", "ready"));
    expect(state.allReady).toBe(false);
